...
import { apiFetch, getApiCacheKey, prefetchApi } from "@/lib/deal-api";
import type { BrokerNotificationsPayload } from "@/lib/broker-notifications";
import { invalidateListingCaches } from "@/lib/client-cache";
import { BrokerChatNavigationSummary, BrokerDashboardData, ChatConversationSummary, EnquiryReply, LeadStatus } from "@/lib/deal-types";
import {
  cn,
  formatCurrency,
  formatDate,
...
export type DashboardSectionId = "overview" | "listings" | "enquiries" | "chats" | "requirements" | "profile";
...
type DashboardSectionPayload = Partial<
  Pick<
    BrokerDashboardData,
    "listings" | "enquiries" | "requirements" | "incomingRequirementMatches" | "requirementNotifications" | "areas"
  >
> & {
  chats?: ChatConversationSummary[] | BrokerChatNavigationSummary[];
};
const BROKER_LISTING_FILTER_IDS: BrokerListingFilterId[] = ["all", "active", "pending", "rejected", "inactive"];
const DASHBOARD_SHELL_API_PATH = "/api/dashboard/shell";
const DASHBOARD_SECTION_API_PATHS: Record<DashboardSectionId, string | null> = {
  overview: "/api/dashboard/overview",
  listings: "/api/dashboard/listings",
  enquiries: "/api/dashboard/enquiries",
  chats: "/api/dashboard/chats",
  requirements: "/api/dashboard/requirements",
  profile: null,
};
const DASHBOARD_NOTIFICATIONS_API_PATH = "/api/dashboard/notifications";
const DASHBOARD_SECTION_HREFS: Record<DashboardSectionId, string> = {
  overview: "/dashboard",
...
function mergeDashboardSectionPayload(
  shell: BrokerDashboardData | null,
  sectionPayload: DashboardSectionPayload | null,
  section: DashboardSectionId
): BrokerDashboardData | null {
  if (!shell) {
    return null;
  }
  ...
  return {
    ...shell,
    listings: sectionPayload?.listings || [],
    enquiries: sectionPayload?.enquiries || [],
    chats: section === "requirements" ? [] : (sectionPayload?.chats as ChatConversationSummary[] | undefined) || [],
    requirements: sectionPayload?.requirements || [],
    incomingRequirementMatches: sectionPayload?.incomingRequirementMatches || [],
    requirementNotifications: sectionPayload?.requirementNotifications || [],
    areas: sectionPayload?.areas || shell.areas,
  };
}
...
export function DashboardSectionPage({ section }: { section: DashboardSectionId }) {
  return (
...
function DashboardPageContent({ section }: { section: DashboardSectionId }) {
  ...
  const prefetchedSectionApiPathsRef = useRef(new Set<string>());
  const dashboardSectionApiPath = DASHBOARD_SECTION_API_PATHS[activeSection];
  const fetchDashboardShell = useCallback(() => apiFetch<BrokerDashboardData>(DASHBOARD_SHELL_API_PATH), []);
  const fetchDashboardSection = useCallback(
    () => (dashboardSectionApiPath ? apiFetch<DashboardSectionPayload>(dashboardSectionApiPath) : Promise.resolve({})),
    [dashboardSectionApiPath]
  );
  const {
    data: dashboardShell,
    loading: shellLoading,
    refresh: refreshDashboardShell,
    setData: setDashboardShell,
  } = useSessionQuery<BrokerDashboardData>(getApiCacheKey(DASHBOARD_SHELL_API_PATH), fetchDashboardShell, {
  ...
  const {
    data: dashboardSectionPayload,
    loading: sectionLoading,
    refresh: refreshDashboardSection,
    setData: setDashboardSectionPayload,
  } = useSessionQuery<DashboardSectionPayload>(
    dashboardSectionApiPath ? getApiCacheKey(dashboardSectionApiPath) : "dashboard-section:profile",
    fetchDashboardSection,
    {
      enabled: !loading && canViewDashboard && Boolean(dashboardSectionApiPath),
      ttlMs: 45_000,
      onError: (error) => enqueueSnackbar(error instanceof Error ? error.message : "Failed to load dashboard section.", { variant: "error" }),
    }
  );
  const dashboard = useMemo(
    () => mergeDashboardSectionPayload(dashboardShell, dashboardSectionPayload, activeSection),
    [activeSection, dashboardSectionPayload, dashboardShell]
  );
  const requirementChatGroups = useMemo(
    () =>
      activeSection === "requirements"
        ? (dashboardSectionPayload?.chats as BrokerChatNavigationSummary[] | undefined) || []
        : [],
    [activeSection, dashboardSectionPayload?.chats]
  );
  const pageLoading = shellLoading || (Boolean(dashboardSectionApiPath) && sectionLoading);
  const {
    data: notificationsPayload,
    setData: setNotificationsPayload,
  } = useSessionQuery<BrokerNotificationsPayload>(
    getApiCacheKey(DASHBOARD_NOTIFICATIONS_API_PATH),
    () => apiFetch<BrokerNotificationsPayload>(DASHBOARD_NOTIFICATIONS_API_PATH),
    {
      enabled: !loading && canViewDashboard && activeSection === "overview",
  ...
  const loadDashboard = useCallback(async () => {
    const [nextShell, nextSectionPayload] = await Promise.all([
      refreshDashboardShell(),
      dashboardSectionApiPath ? refreshDashboardSection() : Promise.resolve(dashboardSectionPayload || {}),
    ]);
    const payload = mergeDashboardSectionPayload(nextShell, nextSectionPayload, activeSection);
    if (!payload) {
      throw new Error("Failed to load dashboard.");
    }

    return payload;
  }, [activeSection, dashboardSectionApiPath, dashboardSectionPayload, refreshDashboardSection, refreshDashboardShell]);

  const handleSelectSection = useCallback(
    (section: DashboardSectionId) => {
      const nextHref = DASHBOARD_SECTION_HREFS[section];
  ...
  const handlePrefetchSection = useCallback(
    (section: DashboardSectionId) => {
      const href = DASHBOARD_SECTION_HREFS[section];
      const apiPath = DASHBOARD_SECTION_API_PATHS[section];

      router.prefetch(href);

      if (!apiPath || !canViewDashboard || loading || section === activeSection || prefetchedSectionApiPathsRef.current.has(apiPath)) {
        return;
      }

      prefetchedSectionApiPathsRef.current.add(apiPath);
      prefetchApi<DashboardSectionPayload>(apiPath, {}, { ttlMs: 45_000 });
    },
    [activeSection, canViewDashboard, loading, router]
  );
  ...
  const handleSelectListingStatusFilter = (filter: BrokerListingFilterId) => {
    ...
    const listingPathname = DASHBOARD_SECTION_HREFS.listings;
    const nextHref = nextQuery ? `${listingPathname}?${nextQuery}` : listingPathname;
  ...
  useEffect(() => {
    ...
    Object.values(DASHBOARD_SECTION_HREFS).forEach((href) => {
      router.prefetch(href);
    });
  }, [canViewDashboard, loading, router]);
  ...
  const sendEnquiryReply = async () => {
    ...
    try {
      ...
      setDashboardSectionPayload((current) => {
  ...
  useEffect(() => {
    ...
    prefetchedSectionApiPathsRef.current.clear();
    setListingStatusFilter(getListingStatusFilterFromSearchParam(requestedListingStatus));
    ...
    replySendingRef.current = false;
    setReplySending(false);
  }, [requestedListingStatus, sessionUserId]);
  ...
  return (
    <AppShell hidePageHeader mainClassName="!max-w-none !px-0 !pt-0">
      <BrokerDashboardReference
        ...
        notificationIsLoadingMore={notificationIsLoadingMore}
        ...
        onSelectSection={handleSelectSection}
        onPrefetchSection={handlePrefetchSection}
      ...
      >
        ...
        {activeSection === "requirements" ? (
          <BrokerRequirementsWorkspace
            chatGroups={requirementChatGroups}
            requirements={dashboard.requirements}
