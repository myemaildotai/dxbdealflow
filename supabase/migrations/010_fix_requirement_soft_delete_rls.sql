...
BEGIN
  ...
  IF next_deleted_at IS NOT NULL THEN
    RETURN next_is_active = FALSE
      AND next_deactivated_by IS NOT NULL
      AND (
        current_requirement.deactivated_by IS NULL
        OR next_deactivated_by = current_requirement.deactivated_by
        OR next_deactivated_by = 'broker'
      );
  END IF;
